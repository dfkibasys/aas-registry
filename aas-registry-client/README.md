...
```java
new MatchQuery().path(AasRegistryPaths.submodelDescriptors().description().text()).value("robot");
```
...
```java
new RegExQuery().path(AasRegistryPaths.submodelDescriptors().description().text()).value("r[ob]{3}t");
```
...
public class RegistryAndDiscoveryInterfaceApiExample {
    ...
    public static void main(String[] args) {
		...
		body.setQuery(new MatchQuery().path(path).value("de-DE"));
