...
public class DotAASRegistryProxy implements IAASRegistry {
	...
	public void register(IIdentifier aasIdentifier, SubmodelDescriptor submodelDescriptor) throws ProviderException {
		org.eclipse.basyx.aas.registry.model.SubmodelDescriptor toPost = DescriptorConversions
				.toDotaasSubmodelDescriptor(submodelDescriptor);
		try {
	...
	public AASDescriptor lookupAAS(IIdentifier aasIdentifier) throws ProviderException {
		try {
			AssetAdministrationShellDescriptor response = client
					.getAssetAdministrationShellDescriptorById(aasIdentifier.getId());
	...
	public SubmodelDescriptor lookupSubmodel(IIdentifier aasIdentifier, IIdentifier submodelIdentifier)
		...
		try {
			org.eclipse.basyx.aas.registry.model.SubmodelDescriptor response = client
					.getSubmodelDescriptorById(aasIdentifier.getId(), submodelIdentifier.getId());
			return DescriptorConversions.toBasyxSubmodelDescriptor(response);
		} catch (NotFound ex) {
			throw new ResourceNotFoundException(String.format("Could not retrieve AAS %s or submodel %s",
					aasIdentifier.getId(), submodelIdentifier.getId()));
		} catch (RestClientException ex) {
			throw new ProviderException(ex);
		}
	}
